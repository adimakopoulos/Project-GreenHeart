...
public class Mouse : MonoBehaviour
{
    ...
    void setTileDir()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (mono_BoardCreate.map.getTileFromMap(clickPoint) != null && mono_BoardCreate.map.getTileFromMap(clickPoint).Owner == mono_playerManager.p1)//&& mono_BoardCreate.map.getTileFromMap(clickPoint).Owner == player1
            {
    ...
    private bool checkMoveValidity(Tile _originalTile, Tile _nTile, Tile.TileMovementDirection intendedDir)
    {
        ...
        {
            if (_nTile.Type == Tile.TileType.Rock) {
                return false;
            }
