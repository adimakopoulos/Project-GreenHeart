...
namespace WorldBuilder
{
    ...
    public class Board
    {
        ...
        public Board(int width, int height )
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < height; z++)
                {
                    ...
                    tiles[x, z].getGoTile().transform.SetParent(GameObject.Find("go_Board").transform, false);//make all the tiles a "Parent of go_Bord in the unity Hierarchy"

                }
            }

            //After the world has been initialized we can set neighboring tiles
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < height; z++)
                {
        ...
        public void enableTiles()
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < height; z++)
                {
                    ...
                    tiles[x, z].getGoTile().SetActive(true);
        ...
        public Tile getTileFromMap(int x, int z)
        {
